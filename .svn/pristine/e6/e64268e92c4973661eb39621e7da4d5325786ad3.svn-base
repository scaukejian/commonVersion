...
import com.yuezhi.ws.utils.EdateBeaUtils;
import com.yuezhi.ws.utils.PasswordDigester;

/**
 * 
 * @author zixiaobang time 2015 05 22
 *
 */
public class EmployeeVO implements Serializable {
	...
	public void vo2Domain(Employee domain) {
		try {
			EdateBeaUtils.copyProperties(domain, this);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void domain2Vo(Employee domain) {
		try {
			EdateBeaUtils.copyProperties(this, domain);
		} catch (Exception e) {
			e.printStackTrace();
			throw new BusinessException("获取员工信息失败");
		}
