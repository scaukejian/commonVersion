...
util.fileDownload = function(url, $from) {
	...
	var loadObj = $.Prompt('<div id="edate_loading" class="loading spin"></div>', 0, {
		bg : true
	}); 
...
util.edateAjax = function(param) {
	...
	obj.complete = function(request, textStatus) {
		...
		if (params.tokenKey) {
			delete edateToken[params.tokenKey];
		}
	...
	if (params.loading) {
		loadObj = $.Prompt('<div id="edate_loading" class="loading spin"></div>', 0, {
			bg : true
		}); 
